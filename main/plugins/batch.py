    ...
    async with gagan.conversation(event.chat_id) as conv: 
        if not s:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="t.me/nxjsjsjjssj")]])
    ...
    for i in range(len(ids)):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/chhfdthbvvffg")]])
            ...
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/xnxnsjsksksks")]])
        n = i + 1
        if n == len(ids):
            return -2
...
TEXT = "👋 Hi, This is 'Paid Restricted Content Saver' bot Made with ❤️ by __**Remo**__."
...
async def start_command(event):
    ...
    buttons = [
        [Button.inline("Cancel", data="cancel"),
         Button.inline("Cancel", data="cancel")],
        [Button.url("Join Channel", url="t.me/biksitlogs")]
    ]
